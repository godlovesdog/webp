...
{/*{{{*/
    ...
    if (blob == NULL) 
    {
        fprintf(stderr, "No blob specified!\n");
        goto Error;
    }
    ...
    {
        fprintf(stderr, "No output buffer specified.\n");
        goto Error;
    }
...
{
    char filename[] = "/home/ben/t006607417acbbb77e6.jpg";
    uint8_t *out = malloc(1024*10);
    out_buf_t out_buf;
    out_buf.start = out;
    out_buf.len = 0;
    char out_file[] = "/home/ben/f.webp";
    FILE* in_file = fopen(filename, "rb");
    unsigned char *blob = malloc(1024*10);
    int cnt = fread(blob,1,10*1024,in_file);
    cwebp(blob,cnt,&out_buf,100,200,200);
    free(blob);
    FILE* o = fopen(out_file,"w+");
    cnt = fwrite(out_buf.start,1,out_buf.len,o);
    printf("%d\n",cnt);
    free(out);
    fclose(in_file);
    fclose(o);
